---
title: MILC CUDA 12 QUDA ks_spectrum stack on Perlmutter
...
# MILC CUDA 12 QUDA `ks_spectrum` stack on Perlmutter
...
The first validation harness searched for `FLTIME HISQ QUDA D`, omitting the colon,
timing fields, and parentheses in the literal output. It therefore exited one after the
application step had exited zero and produced complete numerical output. Record the wrapper
and payload outcomes separately; the wrapper false negative does not invalidate the
application result.
