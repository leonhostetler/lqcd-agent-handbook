...
|---|---|
| `coarsest_global_volume` and coarsest-cell aspect | The three lattice spacings above. Only the 0.09-fm population broadly scanned decompositions. The ordering is established; `coarsest_global_volume >= 10000` and aspect `<= 1.5` are provisional screens, not fitted failure boundaries. A later four-level 0.09-fm population converged below the volume cutoff while preserving its ordering; see [`hierarchy-and-setup.md`](hierarchy-and-setup.md). |
...
| TRLM restart bands | 318 completed deflated rows at 0.04 and 0.06 fm with production residual request `1e-8`; 324 rows when the six completed 0.09-fm rows are included. The stable `4...9` middle band comes from the two working spacings. The six 0.09-fm rows in this population support only the low-restart warning, and a later and larger 0.09-fm population did not reproduce that warning's direction; see [`coarse-deflation.md`](coarse-deflation.md). |
| Level-1 setup cap | Rows with both the terminal level-1 setup iterations and matching `setup_maxiter 1`. The two working spacings sampled the healthy side of one knee; the 0.09-fm rows were at or near the cap. **This population was screened with a mean-to-cap ratio, `rho_setup < 0.5`, which the current contract replaces with `setup_l1_capped_fraction`.** The two are not interconvertible without the per-stream counters, which are not published here, so no fraction-form band is carried across from this corpus. |
