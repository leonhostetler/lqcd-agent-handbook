...
| # | Owed |
|---|---|
| X.1 | `tools/gpu_profile/diagnostics.py` notes N4 and R4 recommend re-profiling with `--gpu-metrics-device=all` so memory- versus compute-bound classification need not "use heuristics only" — a reading `modes/performance.md` and [§profile-analysis](ARCHITECTURE.md#profile-analysis) rule out of a tracer entirely. The note invites the failure the mode exists to prevent. `--gpu-metrics-device` is the sampled middle tier of the counter question parked in [§deferred-decisions](#deferred-decisions), and whether those device-wide samples support any part of the classification was never verified. The fix does not wait on that decision: the note must stop recommending a remedy for a question the mode rules out, whether or not a counter route ever exists |
